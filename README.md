...
The public VPS also has an nftables allowlist generated by
`/usr/local/sbin/update-cloudflare-nft` on a daily systemd timer. If
`http://77.42.40.7:18080/` times out while the container is up and local
`curl 127.0.0.1:18080` works on the VPS, make sure that generator and
`/etc/nftables.d/seedbuster.nft` both include:
