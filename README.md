...
- **Python 3.8+** (only needed for Python verifier path below)

## Rust-Native Verifier (New)

If you want a standalone executable-style verifier path (no Python runtime), use the Rust CLI in [`rust-native-verifier/`](rust-native-verifier).

### Download executable (recommended)
- Use GitHub Releases artifacts generated by `.github/workflows/rust-native-release.yml`.
...
- Each artifact includes:
  - `rust-native-verifier` binary (`.exe` on Windows)
  - `run-verifier.bat` (Windows helper)
  - `run-verifier.sh` (macOS/Linux helper)
  - `README-RUN.txt`

### Build from source
```bash
cd rust-native-verifier
cargo build --release
```

### Run (Auto detect datadir and node type)
```bash
./target/release/rust-native-verifier
```

### Explicit mode examples (optional)
```bash
# Rust node
...
- **Rust nodes**: `~/.rusty-kaspa/kaspa-mainnet/datadir` (the verifier resolves the active `consensus-*` directory automatically)
